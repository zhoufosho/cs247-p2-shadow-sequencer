...
function initializeDOMElements() {
    ...
    blockCanvas.setAttribute('width', 640);
    blockCanvas.setAttribute('height', 640);
    blockCanvas.style.display = SHOW_SHADOW ? 'block' : 'none';
...
function getShadowData() {
    ...
    for (var i=0; i<pixelData.data.length; i=i+4) {
        ...
        var pixelNumber = i/4;
        var rowIndex = Math.floor(pixelNumber/rawCanvas.width);
        var columnIndex = pixelNumber%rawCanvas.width; 
        //if(pixelNumber >= 640)
        ...
        if (distance >= SHADOW_THRESHOLD) {
            ...
            var xPos = Math.floor(columnIndex/blockSize);
            var yPos = Math.floor(rowIndex/blockSize);
           	if(pixelNumber != 0)
