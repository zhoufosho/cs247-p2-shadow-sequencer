...
$(document).ready(function() {	
	var myCoordinates = document.getElementById("coordinates");
	myCoordinates = 10;
...
function initializeDOMElements() {
    ...
    //blockCanvas = shadowCanvas.createLayer();
    ...
    //blockCanvas.style.display = SHOW_ ? 'block' : 'none';
    document.getElementById('capture').appendChild(blockCanvas);
    blockContext = blockCanvas.getContext('2d'); 
    
    rawCanvas = document.createElement('canvas');
    rawCanvas.setAttribute('id', 'rawCanvas');
    rawCanvas.setAttribute('width', width/scaleFactor);
    rawCanvas.setAttribute('height', height/scaleFactor);
    rawCanvas.style.display = SHOW_RAW ? 'none' : 'none';
    ...
    shadowCanvas.setAttribute('width', width/scaleFactor);
    shadowCanvas.setAttribute('height', height/scaleFactor);
    shadowCanvas.style.display = SHOW_SHADOW ? 'none' : 'none';
...
function initializeBlockGrid(){
 blockGrid = new Array(nRows);
  for (var i = 0; i < nRows; i++) {
    blockGrid[i] = new Array(nCols);
...
function getShadowData() {
    ...
    for (var i=0; i<pixelData.data.length; i=i+4) {
        ...
        var threshold = (blockSize*blockSize)/(scaleFactor*scaleFactor*2);
        var pixelNumber = i/4;
        var rowIndex = Math.floor(pixelNumber/shadowCanvas.width);
        var columnIndex = pixelNumber%shadowCanvas.width; 
        if (distance >= SHADOW_THRESHOLD) {
            ...
            var xPos = Math.floor(columnIndex/(shadowCanvas.width/nRows));
            var yPos = Math.floor(rowIndex/(shadowCanvas.height/nCols));
           	if(pixelNumber != 0)
	...
	for (var row = 0; row < nRows; row++){
		for (var col = 0; col < nCols; col++){
			var block = document.getElementById('blockCanvas').getContext('2d');
			block.fillRect(row*blockSize,col*blockSize,blockSize,blockSize);
			if(blockGrid[row][col] >= threshold){	
				block.fillStyle = "rgba(255,193,37,0.1)"; //Fill in orange block
			}
			else{
				block.strokeRect(row*blockSize,col*blockSize,blockSize,blockSize);
				block.clearRect(row*blockSize,col*blockSize,blockSize,blockSize);
			}
		}
	}
